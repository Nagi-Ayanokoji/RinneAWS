...
export function EditPlaylistModal({ isOpen, onClose, playlist, onSuccess }: EditPlaylistModalProps) {
  ...
  useEffect(() => {
    if (playlist) {
      ...
      setPreviewUrl(playlist.cover_url || null);
    }
  }, [playlist]);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await api.put(`/playlists/${playlist.id}`, { name, cover_url });
      onSuccess();
      onClose();
    } catch (error) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="glass-panel w-full max-w-md p-6 rounded-2xl relative shadow-2xl border border-white/10">
        ...
        <h2 className="font-headline-md text-2xl text-on-surface mb-6">Personalizar Playlist</h2>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div>
            <label className="block font-label-sm text-xs text-on-surface-variant mb-1 ml-1">NOMBRE DE LA PLAYLIST</label>
            <input
              type="text"
              value={name}
